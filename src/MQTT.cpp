...
void parse_control(String data);
...
void mqtt_connect()
{
    ...
    while(!mqtt_client.connected() && WiFi.status() == WL_CONNECTED)
    {
        ...
        if(mqtt_client.connect(MQTT_ID, MQTT_USER, MQTT_PASS))
        {
            ...
            mqtt_client.subscribe(MQTT_CONTROL.c_str());
            give_up = false;
        } else {
...
void mqtt_downlink(char* topic, byte* message, unsigned int length)
{
    ...
    } else if(topic_string == MQTT_CONTROL) {
        for(int x = 0; x < length; x++)
        {
            mqtt_data += (char)message[x];
        }
        parse_control(mqtt_data);
    } else {
...
/**
 * @brief Parse incoming MQTT data for control
 * 
 * @param data 
 */
void parse_control(String data)
{
    ...
    /** CMD 0: ON */
    case 0:
        pin_on(stoi(seglist[1]));
    break;
    /** CMD 1: OFF */
    case 1:
        pin_off(stoi(seglist[1]));
    break;
    }
}
